...
//                                           (jeton de 16 caractères généré automatiquement, unique)
...
// Le routeur (/s/{jeton}) et l'authentification exigent tous deux 12 à 64
// caractères : un jeton plus court produit un lien qui ne connecte personne.
const LONGUEUR_JETON = 16;

/** Jeton aléatoire [a-zA-Z0-9], garanti unique dans la base. */
function genererJeton(existants) {
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
  for (let essai = 0; essai < 50; essai++) {
    let jeton = '';
    for (let i = 0; i < LONGUEUR_JETON; i++) jeton += chars[crypto.randomInt(chars.length)];
