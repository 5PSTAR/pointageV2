...
export default function Pointages() {
  ...
  // Seuls les hôtels réellement exploitables sont proposés : rattacher un
  // pointage à une fiche sans nom le laisserait hors facturation.
  const [hotels, setHotels] = useState([]);
  useEffect(() => {
    api('/api/admin/referentiels')
      .then((d) => setHotels((d.hotels || []).filter((h) => h.nom)))
      .catch(() => {});
  }, []);
  ...
  const charger = () => {
    ...
    api(`/api/admin/pointages?${p}`).then((d) => {
      ...
      if (cible) {
        ...
        if (pt) setEdition({ ...pt, hotelId0: pt.hotelId });
      }
  ...
  async function corriger() {
    try {
      // L'hôtel n'est transmis que s'il a changé : sinon un pointage rattaché à
      // une fiche défectueuse ne pourrait plus être corrigé du tout.
      const hotelChange = edition.hotelId !== edition.hotelId0;
      await api('/api/admin/pointages', { method: 'PATCH', body: {
        ...
        hotelId: hotelChange ? edition.hotelId : undefined,
      }});
      setEdition(null);
      setInfo(hotelChange
        ? 'Pointage rattaché — il entre maintenant dans la facturation.'
        : 'Pointage corrigé — la modification est tracée dans l\'observation.');
      charger();
    } catch (e) { setErreur(e.message); }
  }
            ...
            <tbody>
              {data.pointages.map((p) => (
                <tr key={p.id}>
                  ...
                  <td>{p.hotel || <span className="chip orange">⚠ à rattacher</span>}</td>
                  <td>{p.prestation}</td>
                  ...
                  <td><button className="btn mini" onClick={() => setEdition({ ...p, hotelId0: p.hotelId })}>✏️ Rectifier</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {edition && (
        <div className="voile" onClick={(e) => e.target === e.currentTarget && setEdition(null)}>
          <div className="modale">
            ...
            <p className="muted petit" style={{ marginBottom: 14 }}>{edition.hotel || 'Hôtel à rattacher'} · {edition.prestation}</p>
            <div className="champ" style={{ marginBottom: 10 }}>
              <label>Hôtel</label>
              <select style={{ width: '100%' }} value={edition.hotelId || ''}
                onChange={(ev) => setEdition({ ...edition, hotelId: ev.target.value })}>
                {!hotels.some((h) => h.id === edition.hotelId) && (
                  <option value={edition.hotelId || ''}>— à rattacher —</option>
                )}
                {hotels.map((h) => <option key={h.id} value={h.id}>{h.nom}</option>)}
              </select>
              {!edition.hotel && (
                <p className="muted petit" style={{ marginTop: 5 }}>
                  Ce pointage n'est rattaché à aucun hôtel exploitable : il est exclu de la facturation tant qu'il n'en a pas.
                </p>
              )}
            </div>
            <div className="champ" style={{ marginBottom: 10 }}>
