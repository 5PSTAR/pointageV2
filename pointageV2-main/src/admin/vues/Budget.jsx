...
const IcoExcel = <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>;
const IcoPdf = <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/></svg>;

/** Lien de téléchargement, neutralisé quand il n'y a rien à exporter. */
function BoutonExport({ href, actif, classe, icone, libelle, titre }) {
  if (!actif) {
    return (
      <span className={`btn ${classe}`} aria-disabled="true" title={titre}
            style={{ opacity: 0.5, cursor: 'default', pointerEvents: 'none' }}>
        {icone}{libelle}
      </span>
    );
  }
  return <a className={`btn ${classe}`} href={href} title={titre}>{icone}{libelle}</a>;
}
...
export default function Budget() {
  ...
  // L'export suit la sélection : une salariée précise, sinon le consolidé.
  const lienExport = (format) =>
    `/api/admin/budget?mois=${mois}${salarie ? `&salarie=${salarie}` : ''}&format=${format}`;
  const exportable = salarie ? Boolean(ligneChoisie) : Boolean(data?.lignes.length);
  const nomChoisi = ligneChoisie?.salarie || profil?.nom || 'cette salariée';
  const moisLisible = new Date(mois + '-01T12:00:00Z')
    .toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' });
  const portee = salarie
    ? (ligneChoisie ? `Rémunération de ${nomChoisi} — ${moisLisible}` : `Aucun pointage à exporter pour ${nomChoisi}`)
    : (data?.lignes.length ? `Toutes les salariées — ${moisLisible}` : 'Aucun pointage à exporter sur ce mois');
  ...
  return (
    <>
        ...
        <div className="filtres" style={{ marginBottom: 0 }}>
          <div className="champ"><label>Salariée</label>
            <select value={salarie} onChange={(e) => setSalarie(e.target.value)}>
              ...
              {(refs?.salaries || []).map((s) => <option key={s.id} value={s.id}>{s.nom}</option>)}
            </select>
          </div>
          <SelecteurMois valeur={mois} onChange={setMois} />
          <div className="pousse">
            <BoutonExport href={lienExport('xlsx')} actif={exportable} classe="" icone={IcoExcel}
                          libelle={salarie ? 'Excel (cette salariée)' : 'Excel (consolidé)'} titre={portee} />
            <BoutonExport href={lienExport('pdf')} actif={exportable} classe="bleu" icone={IcoPdf}
                          libelle="PDF" titre={portee} />
          </div>
        </div>
      </div>
      ...
      {/* ── Salariée sélectionnée, mais rien à facturer ce mois-ci ── */}
      {data && salarie && !ligneChoisie && (
        <div className="carte" style={{ textAlign: 'center', padding: '28px 16px' }}>
          <p style={{ margin: 0, fontWeight: 600 }}>Aucun pointage terminé en {moisLisible}</p>
          <p className="muted" style={{ margin: '6px 0 0' }}>
            {nomChoisi} n'a pas de mission terminée sur ce mois — rien à payer, rien à exporter.
          </p>
        </div>
      )}
            ...
            <tbody>
              ...
              {!data.lignes.length && (
                <tr><td colSpan="6" className="muted" style={{ textAlign: 'center', padding: 24 }}>
                  Aucun pointage terminé en {moisLisible}.
                </td></tr>
              )}
              {Boolean(data.lignes.length) && (
                <tr className="total-ligne">
