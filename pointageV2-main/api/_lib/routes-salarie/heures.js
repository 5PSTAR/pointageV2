...
import { T, lister, referentiels, lirePointage, F, aujourdhuiParis, lundiCourant, moisDecale, echapper, envoyerErreur } from '../airtable.js';
...
export default async function handler(req, res) {
  try {
    ...
    // La pause du client se retranche AVANT tout cumul : la paie la déduit, donc
    // l'application doit annoncer les mêmes heures. Lire 9 h sur son téléphone
    // et en toucher 8,5 est le plus sûr moyen de ne plus croire l'outil.
    const pointages = appliquerPauseParClient(bruts.map((x) => lirePointage(x, refs)), refs);
